...
def apply_device_overrides(config_dict: dict, capabilities: DeviceCapabilities) -> dict:
    ...
    if stt_processor not in capabilities.allowed_stt:
        if show_warnings:
            queue_message(
                f"WARNING: STT '{stt_processor}' not supported on {capabilities.profile.value}, using '{capabilities.fallback_stt}'"
            )
        config_dict["STT"]["stt_processor"] = capabilities.fallback_stt

    tts_option = (
        config_dict["TTS"].ttsoption
        if hasattr(config_dict["TTS"], "ttsoption")
        else config_dict["TTS"]["ttsoption"]
    )
    if tts_option not in capabilities.allowed_tts:
        if show_warnings:
            queue_message(
                f"WARNING: TTS '{tts_option}' not supported on {capabilities.profile.value}, using '{capabilities.fallback_tts}'"
            )
        if hasattr(config_dict["TTS"], "ttsoption"):
    ...
    if vad_method not in capabilities.allowed_vad:
        if show_warnings:
            queue_message(
                f"WARNING: VAD '{vad_method}' not supported on {capabilities.profile.value}, using '{capabilities.fallback_vad}'"
            )
    ...
    if wake_processor not in capabilities.allowed_wake:
        if show_warnings:
            queue_message(
                f"WARNING: Wake word '{wake_processor}' not supported on {capabilities.profile.value}, using '{capabilities.fallback_wake}'"
            )
    ...
    if config_dict["LLM"]["contextsize"] > capabilities.max_context_size:
        if show_warnings:
            queue_message(
                f"WARNING: Context size {config_dict['LLM']['contextsize']} exceeds max {capabilities.max_context_size} for {capabilities.profile.value}"
            )
    ...
    def validate(self) -> bool:
        if self.ttsoption == "azure":
            if not (self.azure_api_key and self.azure_region):
                queue_message(
                    "ERROR: Azure API key and region are required for Azure TTS"
                )
                return False
        elif self.ttsoption == "elevenlabs":
            if not self.elevenlabs_api_key:
                queue_message(
                    "ERROR: ElevenLabs API key is required for ElevenLabs TTS"
                )
                return False
...
def load_config():
    ...
    if missing_sections:
        queue_message(
            f"ERROR: Missing sections in config.ini: {', '.join(missing_sections)}"
        )
        sys.exit(1)
    ...
    if "PERSONA" in persona_config:
        persona_traits = {
            key: int(value) for key, value in persona_config["PERSONA"].items()
        }
    else:
    ...
    if config.has_section("DEVICE") and config.has_option(
        "DEVICE", "raspberry_version"
    ):
    ...
    config_dict = {
        ...
        "CONTROLS": {
            ...
            "swap_turn_directions": config.getboolean(
                "CONTROLS", "swap_turn_directions", fallback=False
            ),
        ...
        "RAG": {
            ...
            "recency_boost_days": config.getint(
                "RAG", "recency_boost_days", fallback=7
            ),
        ...
        "STABLE_DIFFUSION": {
            ...
            "denoising_strength": float(
                config["STABLE_DIFFUSION"]["denoising_strength"]
            ),
        ...
        "UI": {
            ...
            "screensaver_list": _parse_screensaver_list(
                config.get("UI", "screensaver_list", fallback="random")
            ),
            "show_time": config.getboolean("UI", "show_time", fallback=True),
            "ampm_format": config.getboolean("UI", "ampm_format", fallback=True),
            "screensaver_cycle_interval": config.getint(
                "UI", "screensaver_cycle_interval", fallback=300
            ),
        },
        "BATTERY": {
            "battery_capacity_mAh": int(config["BATTERY"]["battery_capacity_mAh"]),
            "battery_initial_voltage": float(
                config["BATTERY"]["battery_initial_voltage"]
            ),
            "battery_cutoff_voltage": float(
                config["BATTERY"]["battery_cutoff_voltage"]
            ),
            "auto_shutdown": config.getboolean("BATTERY", "auto_shutdown"),
        },
        "KASA": {
            "espresso_machine": config.get("KASA", "espresso_machine", fallback=""),
            "coffee_bar_lights": config.get("KASA", "coffee_bar_lights", fallback="")
        },
        "MISC": {
...
def get_api_key(llm_backend: str) -> str:
    ...
    if llm_backend not in backend_to_env_var:
        print(
            f"WARNING: Unsupported LLM backend '{llm_backend}', skipping API key lookup."
        )
        return ""
    api_key = os.getenv(backend_to_env_var[llm_backend])
    if not api_key:
        print(
            f"WARNING: No API key found for '{llm_backend}' (env var: {backend_to_env_var[llm_backend]}). LLM features will be unavailable."
        )
        return ""
    return api_key
...
def reload_persona_settings():
    ...
    try:
        ...
        persona_path = os.path.join(
            base_dir, "character", character_name, "persona.ini"
        )
        if not os.path.exists(persona_path):
...
CONFIG_METADATA = {
    ...
    "CHAR": {
        ...
        "character_card_path": {"description": "Path to character card file"},
        "user_name": {"description": "Your name (how TARS addresses you)"},
        "user_details": {"description": "Details about you for context"},
        "responses": {"description": "Random idle responses (JSON array)"},
        "thinking_responses": {"description": "Thinking filler responses (JSON array)"},
        "latitude": {"description": "TARS latitude (e.g. 45.5017)"},
        "longitude": {"description": "TARS longitude (e.g. -73.5673)"},
        "location_name": {
    ...
    "CONTROLS": {
        ...
        "controller_name": {"description": "Name of the connected controller"},
        "enabled": {"description": "Enable controller input"},
        "voicemovement": {"description": "Enable voice-controlled movement"},
        "swap_turn_directions": {"description": "Swap left/right turn directions"},
        "invert_y": {
    ...
    "STT": {
        ...
        "wake_word": {"description": "Phrase to activate listening"},
        "wake_word_processor": {
            "options": ["picovoice", "fastrtc", "atomik"],
            "description": "Wake word detection engine",
        },
        "sensitivity": {"description": "Wake word sensitivity (1-10)"},
        "stt_processor": {
            "options": [
                "vosk",
                "faster-whisper",
        ...
        "external_url": {"description": "URL for external STT server"},
        "whisper_model": {
            "options": ["tiny", "base", "small", "medium", "large"],
            "description": "Whisper model size",
        },
        "vosk_model": {"description": "Vosk model name"},
        "use_indicators": {"description": "Play audio indicators for listening state"},
        "vad_method": {
            "options": ["silero", "rms"],
    ...
    "LLM": {
        ...
        "base_url": {"description": "API base URL"},
        "openai_model": {"description": "OpenAI model name"},
        "grok_model": {"description": "Grok model name"},
        "override_encoding_model": {
            "options": ["cl100k_base", "p50k_base", "r50k_base", "gpt2"],
            "description": "Token encoding model override",
        },
        "contextsize": {"description": "Maximum token context size"},
        "max_tokens": {"description": "Maximum tokens per response"},
        "temperature": {"description": "Randomness (0.0-1.0, higher = more random)"},
        "top_p": {"description": "Nucleus sampling threshold"},
        "seed": {"description": "Random seed (-1 for random)"},
        "systemprompt": {"description": "System prompt defining LLM behavior"},
    },
    "VISION": {
        "__description__": "Vision/image recognition configuration",
        "enabled": {"description": "Enable vision module"},
        "server_hosted": {"description": "Vision server is hosted locally"},
        "base_url": {"description": "Vision server API URL"},
    },
    "EMOTION": {
        "__description__": "Emotion detection for avatars",
        "enabled": {"description": "Enable emotion detection"},
        "emotion_model": {"description": "HuggingFace model for emotion analysis"},
    },
    "TTS": {
        ...
        "ttsurl": {"description": "TTS server URL (for alltalk)"},
        "tts_voice": {"description": "Voice name (for azure/alltalk)"},
        "azure_region": {
            "options": [
                "eastus",
                "eastus2",
        ...
        "elevenlabs_voice_id": {"description": "ElevenLabs voice ID"},
        "elevenlabs_model": {
            "options": [
        ...
        "minimax_voice_id": {"description": "Minimax voice ID"},
        "minimax_model": {
            "options": [
                "speech-2.6-turbo",
                "speech-2.8-turbo",
        ...
        "toggle_charvoice": {"description": "Use character-specific voice settings"},
    },
    "UI": {
        "__description__": "Graphical interface settings",
        "UI_enabled": {"description": "Enable the visual UI"},
        "use_camera_module": {"description": "Enable camera"},
        "show_mouse": {"description": "Show software mouse cursor"},
        "fullscreen": {"description": "Run in fullscreen mode"},
        "font_size": {"description": "Font size (9-20)"},
        "screensaver_timer": {
        ...
        "show_time": {"description": "Show time on screensaver"},
        "ampm_format": {"description": "Use AM/PM time format"},
        "show_cpu_temp": {"description": "Show CPU temperature"},
        "target_fps": {"description": "UI refresh rate (FPS)"},
    },
    "RAG": {
        ...
        "top_k": {"description": "Number of documents to retrieve"},
        "context_window": {"description": "Memories before/after each match"},
        "max_memories": {"description": "Max results to expand"},
        "recency_boost_days": {"description": "Days to boost recent chats"},
        "enable_topic_tracking": {"description": "Enable long-term memory"},
    },
    "BATTERY": {
        "__description__": "Battery monitoring configuration",
        "battery_capacity_mAh": {"description": "Battery capacity (mAh)"},
        "battery_initial_voltage": {"description": "Fully charged voltage (e.g. 12.6)"},
        "battery_cutoff_voltage": {"description": "Minimum safe voltage (e.g. 10.0)"},
        "auto_shutdown": {"description": "Shutdown system when battery is critical"},
    },
    "MISC": {
        "__description__": "Miscellaneous settings",
        "ventilate": {"description": "Enable fan/ventilation control"},
    },
}
...
def update_config_from_web_ui(data: dict, create_backup: bool = True) -> dict:
    ...
    try:
        ...
        success, message, actions_taken = cms.update_config_programmatically(
            data, create_backup
        )
        return {
            "success": success,
