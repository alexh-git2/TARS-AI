    ...
    if os.path.exists(INI_PATH):
        try:
            ...
            if config.has_section("Location"):
                defaults["latitude"] = config.getfloat(
                    "Location", "latitude", fallback=defaults["latitude"]
                )
                defaults["longitude"] = config.getfloat(
                    "Location", "longitude", fallback=defaults["longitude"]
                )
                defaults["timezone"] = config.get(
                    "Location", "timezone", fallback=defaults["timezone"]
                )
                defaults["location_name"] = config.get(
                    "Location", "location_name", fallback=defaults["location_name"]
                )

            if config.has_section("Language"):
                defaults["language"] = config.get(
                    "Language", "language", fallback=defaults["language"]
                )
                defaults["country"] = config.get(
                    "Language", "country", fallback=defaults["country"]
                )

            if config.has_section("Weather"):
                defaults["temp_unit"] = config.get(
                    "Weather", "temp_unit", fallback=defaults["temp_unit"]
                )
                defaults["weather_update_interval"] = config.getint(
                    "Weather",
                    "update_interval",
                    fallback=defaults["weather_update_interval"],
                )

            if config.has_section("News"):
                defaults["news_update_interval"] = config.getint(
                    "News", "update_interval", fallback=defaults["news_update_interval"]
                )
                defaults["max_news_items"] = config.getint(
                    "News", "max_items", fallback=defaults["max_news_items"]
                )

            if config.has_section("Display"):
                defaults["show_weather"] = config.getboolean(
                    "Display", "show_weather", fallback=defaults["show_weather"]
                )
                defaults["show_forecast"] = config.getboolean(
                    "Display", "show_forecast", fallback=defaults["show_forecast"]
                )
                defaults["show_sun_moon"] = config.getboolean(
                    "Display", "show_sun_moon", fallback=defaults["show_sun_moon"]
                )
                defaults["show_news"] = config.getboolean(
                    "Display", "show_news", fallback=defaults["show_news"]
                )
                defaults["show_clock"] = config.getboolean(
                    "Display", "show_clock", fallback=defaults["show_clock"]
                )
                defaults["show_date"] = config.getboolean(
                    "Display", "show_date", fallback=defaults["show_date"]
                )
                defaults["show_quotes"] = config.getboolean(
                    "Display", "show_quotes", fallback=defaults["show_quotes"]
                )
                defaults["background_animation"] = config.getboolean(
                    "Display",
                    "background_animation",
                    fallback=defaults["background_animation"],
                )
                defaults["scroll_speed"] = config.getint(
                    "Display", "scroll_speed", fallback=defaults["scroll_speed"]
                )

            if config.has_section("QuoteMode"):
                defaults["quote_mode"] = config.get(
                    "QuoteMode", "mode", fallback=defaults["quote_mode"]
                )
            ...
            if config.has_section("Calendar"):
                ...
                if ics_urls_str:
                    defaults["calendar_ics_urls"] = [
                        url.strip() for url in ics_urls_str.split(",") if url.strip()
                    ]
                defaults["calendar_days_ahead"] = config.getint(
                    "Calendar", "days_ahead", fallback=defaults["calendar_days_ahead"]
                )
                defaults["calendar_max_events"] = config.getint(
                    "Calendar", "max_events", fallback=defaults["calendar_max_events"]
                )
                defaults["calendar_update_interval"] = config.getint(
                    "Calendar",
                    "update_interval",
                    fallback=defaults["calendar_update_interval"],
                )

            if config.has_section("Feed"):
                defaults["feed_mode"] = config.get(
                    "Feed", "mode", fallback=defaults["feed_mode"]
                )
...
def build_news_feeds(config):
    ...
    feeds = [
        (
            "Local",
            f"https://news.google.com/rss/headlines/section/geo/{geo}?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
        (
            "Top Stories",
            f"https://news.google.com/rss?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
        (
            "Technology",
            f"https://news.google.com/rss/headlines/section/topic/TECHNOLOGY?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
        (
            "Science",
            f"https://news.google.com/rss/headlines/section/topic/SCIENCE?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
        (
            "Business",
            f"https://news.google.com/rss/headlines/section/topic/BUSINESS?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
        (
            "World",
            f"https://news.google.com/rss/headlines/section/topic/WORLD?hl={lang}-{country}&gl={country}&ceid={country}:{lang}",
        ),
    ]
...
QUOTES = {
    ...
    "fr": [
        (
            "L'amour est la seule chose capable de transcender le temps et l'espace.",
            "Interstellar",
        ),
        (
            "Nous levions les yeux vers le ciel pour nous émerveiller des étoiles. Maintenant, nous regardons le sol et nous inquiétons.",
            "Interstellar",
        ),
        (
            "L'humanité est née sur Terre. Elle n'était pas destinée à y mourir.",
            "Interstellar",
        ),
        (
            "N'entre pas docilement dans cette nuit éternelle. Résiste à l'extinction de la lumière.",
            "Interstellar",
        ),
        (
            "Le temps est relatif. Il peut s'étirer et se contracter, mais jamais reculer.",
            "Interstellar",
        ),
        (
            "Être parent, c'est devenir le fantôme du futur de ses enfants.",
            "Interstellar",
        ),
        ("Nous sommes des explorateurs, pas des gardiens.", "Interstellar"),
        (
            "L'univers est immense, mais la volonté humaine l'est davantage.",
            "Science-fiction",
        ),
        (
        ...
        (
            "Les étoiles ne sont pas plus grandes, mais elles brillent davantage.",
            "Interstellar",
        ),
    ],
    "es": [
        (
            "El amor es lo único capaz de trascender el tiempo y el espacio.",
            "Interstellar",
        ),
        (
            "Antes mirábamos al cielo y soñábamos con las estrellas. Ahora miramos al suelo y nos preocupamos.",
            "Interstellar",
        ),
        (
        ...
        (
            "No entres dócil en esa noche eterna. Lucha contra la muerte de la luz.",
            "Interstellar",
        ),
        ("El tiempo es relativo. Puede estirarse, pero no retroceder.", "Interstellar"),
        (
            "Ser padre es convertirse en el fantasma del futuro de tus hijos.",
            "Interstellar",
        ),
        ("Somos exploradores, no cuidadores.", "Interstellar"),
        (
            "El universo es inmenso, pero la voluntad humana lo es aún más.",
            "Ciencia ficción",
        ),
    ...
    "de": [
        ...
        (
            "Früher blickten wir zu den Sternen. Heute schauen wir nach unten und sorgen uns.",
            "Interstellar",
        ),
        (
            "Die Menschheit wurde auf der Erde geboren, nicht um hier zu sterben.",
            "Interstellar",
        ),
        (
            "Geh nicht sanft in diese gute Nacht. Kämpfe gegen das Erlöschen des Lichts.",
            "Interstellar",
        ),
        (
            "Zeit ist relativ. Sie kann sich dehnen, aber nicht zurücklaufen.",
            "Interstellar",
        ),
        (
            "Eltern werden heißt, zum Geist der Zukunft der eigenen Kinder zu werden.",
            "Interstellar",
        ),
        ("Wir sind Entdecker, keine Verwalter.", "Interstellar"),
        (
            "Das Universum ist groß, aber der menschliche Wille ist größer.",
            "Science-Fiction",
        ),
        (
...
class DataFetcher:
    ...
    def _update_loop(self):
        while self.running:
            ...
            if (
                current_time - self.last_weather_update
            ...
            if (
                self.config.FEED_MODE in ("news", "alternate")
                and current_time - self.last_news_update
                > self.config.NEWS_UPDATE_INTERVAL
            ):
                self._fetch_news()
                self.last_news_update = current_time

            if (
                self.config.FEED_MODE in ("calendar", "alternate")
                and self.config.CALENDAR_ICS_URLS
                and current_time - self.last_calendar_update
                > self.config.CALENDAR_UPDATE_INTERVAL
            ):
                self._fetch_calendar()
                self.last_calendar_update = current_time

            if (
                self.config.QUOTE_MODE in ("jokes", "alternate")
                and current_time - self.last_jokes_update > 600
            ):
                self._fetch_jokes()
    ...
    def _fetch_weather(self):
        try:
            ...
            req = urllib.request.Request(
                url, headers={"User-Agent": "DashboardScreensaver/1.0"}
            )
    ...
    def _fetch_news(self):
        ...
        for source_name, feed_url in self.news_feeds:
            try:
                req = urllib.request.Request(
                    feed_url, headers={"User-Agent": "DashboardScreensaver/1.0"}
                )
                with urllib.request.urlopen(req, timeout=10) as response:
                ...
                for item in items[:10]:
                    ...
                    if not image_url:
                        ...
                        if media is not None:
                            ...
                            if "image" in media_type or media.get("url", "").endswith(
                                (".jpg", ".png", ".jpeg", ".webp")
                            ):
                    ...
                    if not image_url:
                        ...
                        if media_group is not None:
                            media_content = media_group.find(
                    ...
                    if not image_url:
                        ...
                        if enclosure is not None:
                            ...
                            if "image" in enc_type or enclosure.get("url", "").endswith(
                                (".jpg", ".png", ".jpeg", ".webp")
                            ):
...
class DashboardAnimation:
    def __init__(self, screen, width, height, show_time=False):
        ...
        self.weather_desc = WEATHER_DESCRIPTIONS.get(
            self.lang, WEATHER_DESCRIPTIONS["en"]
        )
    ...
    def initialize(self):
        ...
        self.panels["quote"] = {
            "x": margin,
            "y": self.layout_height - margin - quote_height,
            "w": panel_width,
            "h": quote_height,
        }
    ...
    def _draw_text(
        self, text, x, y, font_name="small", color=None, center=False, alpha=1.0
    ):
        if color is None:
    ...
    def _render_background_animation(self):
        ...
        for ribbon in self.ribbons:
            ...
            amp_pulse = 0.5 + 0.5 * math.sin(
                self.time * ribbon["amp_vary_speed"] + ribbon["amp_vary_phase"]
            )
            amp_mult = (1 - ribbon["amp_vary_range"]) + ribbon[
                "amp_vary_range"
            ] * amp_pulse

            alpha_pulse = 0.5 + 0.5 * math.sin(
                self.time * ribbon["alpha_speed"] + ribbon["alpha_phase"]
            )
            ribbon_alpha = ribbon["alpha_base"] * (
                (1 - ribbon["alpha_range"]) + ribbon["alpha_range"] * alpha_pulse
            )
            ...
            for i in range(num_points):
                ...
                wave1 = (
                    math.sin(x * ribbon["freq1"] + ribbon["phase"])
                    * ribbon["amplitude1"]
                    * osc1
                    * amp_mult
                )
                wave2 = (
                    math.sin(x * ribbon["freq2"] + ribbon["phase"] * 1.3)
                    * ribbon["amplitude2"]
                    * osc2
                    * amp_mult
                )
                wave3 = (
                    math.sin(x * ribbon["freq3"] + ribbon["phase"] * 0.7)
                    * ribbon["amplitude3"]
                    * osc3
                    * amp_mult
                )
                ...
                thickness_wave = 0.6 + 0.6 * math.sin(
                    (x - thick_t1) * 0.008 + ribbon["thickness_phase"]
                )
                thickness_wave2 = 0.7 + 0.5 * math.sin(
                    (x - thick_t2) * 0.004 + ribbon["thickness_phase"] * 1.5
                )

                thickness = ribbon["thickness_base"] * thickness_wave * thickness_wave2
                thickness = max(thickness, ribbon["thickness_base"] * 0.4)

                blue_amount = 0.5 + 0.5 * math.sin(
                    (x - color_time) * ribbon["color_freq"] + ribbon["color_phase"]
                )
                blue_amount *= 0.5 + 0.5 * math.sin(
                    (x - color_time * 0.7) * ribbon["color_freq"] * 0.5
                    + ribbon["color_phase"] * 1.3
                )
    ...
    def _draw_surface_with_fade(
        self, surface, x, y, fade_top=40, fade_bottom=50, scroll_offset=0
    ):
        ...
        if HAS_NUMPY:
            ...
            effective_fade_top = (
                min(fade_top, scroll_offset) if scroll_offset > 0 else 0
            )
            ...
            for row in range(min(int(fade_bottom), height)):
                ...
                if screen_row >= 0 and screen_row < height:
                    arr[:, screen_row] = (arr[:, screen_row] * alpha_mult).astype(
                        "uint8"
                    )
    ...
    def _render_clock(self):
        ...
        self._draw_text(
            sec_str, panel["x"] + 190, panel["y"] + 45, "small", self.dim_color
        )

        if ampm_str:
            self._draw_text(
                ampm_str, panel["x"] + 190, panel["y"] + 10, "small", self.dim_color
            )

        date_str = format_date_localized(now, self.lang, self.labels)

        self._draw_text(
            date_str, panel["x"] + 270, panel["y"] + 10, "small", self.dim_color
        )
        self._draw_text(
    ...
    def _render_weather(self):
        ...
        if weather:
            ...
            self._draw_text(
                temp_str, panel["x"], panel["y"] + 35, "large", self.text_color
            )
            self._draw_text(
                f"°{unit}", panel["x"] + 165, panel["y"] + 45, "small", self.dim_color
            )
            ...
            right_x = panel["x"] + 270
            self._draw_text(
                condition, right_x, panel["y"] + 40, "small", self.text_color
            )
            ...
            if isinstance(feels_like, (int, float)):
                self._draw_text(
                    f"{self.labels['feels']}: {feels_like:.1f}°{unit}",
                    right_x,
                    panel["y"] + 70,
                    "tiny",
                    self.dim_color,
            ...
            self._draw_text(
                f"{self.labels['humidity']}: {humidity}%",
                right_x,
                panel["y"] + 95,
                "tiny",
                self.dim_color,
    ...
    def _render_forecast(self):
        ...
        self._draw_text(
            self.labels["forecast"], panel["x"], panel["y"], "tiny", self.accent_color
        )
        self._draw_line(
            panel["x"],
            panel["y"] + 25,
        ...
        if forecast and "time" in forecast:
            ...
            for i in range(num_days):
                ...
                self._draw_text(
                    f"{hi_str}/{lo_str}", x + 5, y + 44, "tiny", self.text_color
                )

                if rain and rain > 0:
                    self._draw_text(
                        f"{rain}%", x + 5, y + 66, "tiny", self.accent_color
                    )
        else:
            self._draw_text(
                self.labels["loading"],
    ...
    def _draw_moon_icon(self, cx, cy, radius, phase_index, illumination):
        ...
        if phase_index == 0:
            ...
            for i in range(16, -1, -1):
                angle = -math.pi / 2 + math.pi * i / 16
                glVertex2f(
                    cx + radius * curve * math.cos(angle), cy + radius * math.sin(angle)
                )
            glEnd()
    ...
    def _render_sun_moon(self):
        ...
        self._draw_text(
            self.labels["sunrise"], text_x, panel["y"] + 20, "tiny", self.dim_color
        )
        ...
        self._draw_text(
            self.labels["sunset"], text_x, panel["y"] + 20, "tiny", self.dim_color
        )
    ...
    def _render_news(self):
        ...
        self._draw_text(
            self.labels["news"], panel["x"], panel["y"], "tiny", self.accent_color
        )
        self._draw_line(
            panel["x"],
            panel["y"] + 25,
        ...
        for layout in items_layout:
            ...
            self._draw_text_to_surface(
                feed_surface, item["source"], 0, cur_y, "tiny", self.accent_color
            )
            cur_y += 22

            for line in layout["title_lines"]:
                self._draw_text_to_surface(
                    feed_surface, line, 0, cur_y, "small", self.text_color
                )
                cur_y += 28
            ...
            if layout["desc_lines"]:
                ...
                for line in layout["desc_lines"]:
                    self._draw_text_to_surface(
                        feed_surface, line, 0, cur_y, "tiny", self.dim_color
                    )
                    cur_y += 20

            cur_y += 5
            self._draw_line_to_surface(
                feed_surface, 0, cur_y, surface_width - 20, cur_y, self.dim_color, 0.2
            )

            y_pos += item_height

        self._draw_surface_with_fade(
            feed_surface,
            panel["x"],
            panel["y"] + 35,
            fade_top,
    ...
    def _render_calendar(self):
        ...
        self._draw_text(
            self.labels["calendar"], panel["x"], panel["y"], "tiny", self.accent_color
        )
        self._draw_line(
            panel["x"],
            panel["y"] + 25,
        ...
        if not events:
            if not self.config.CALENDAR_ICS_URLS:
                ...
                if (
                    self.feed_switch_time is None
                    and self.config.FEED_MODE == "alternate"
                ):
                    self.feed_switch_time = self.time + 10
            elif self.data_fetcher.is_calendar_loaded():
                no_events_msg = self.labels["no_events"].format(
                    days=self.config.CALENDAR_DAYS_AHEAD
                )
                self._draw_text(
                    no_events_msg, panel["x"], panel["y"] + 60, "small", self.dim_color
                )
                if (
                    self.feed_switch_time is None
                    and self.config.FEED_MODE == "alternate"
        ...
        for event in events:
            ...
            if start.date() == now.date():
                ...
                day_names = self.labels.get(
                    "days", ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
                )
                months = self.labels.get(
                    "months",
                    [
                        "Jan",
                        "Feb",
            ...
            all_desc_lines = (
                self._wrap_text(description, text_width) if description else []
            )
            ...
            all_people_lines = (
                self._wrap_text(people_str, text_width) if people_str else []
            )
        ...
        for layout in items_layout:
            ...
            self._draw_text_to_surface(
                feed_surface, header, 0, cur_y, "tiny", self.accent_color
            )
            cur_y += 22
            ...
            if layout["people_lines"]:
                for line in layout["people_lines"]:
                    self._draw_text_to_surface(
                        feed_surface, f"👥 {line}", 0, cur_y, "tiny", self.dim_color
                    )
                    cur_y += 20
    ...
    def _render_feed(self):
        ...
        if mode == "news":
            ...
            if self.feed_active == "news":
                self._render_news()
                if (
                    self.news_cycle_complete
                    and self.feed_switch_time is None
                    and not self.feed_startup_grace
                ):
    ...
    def _render_quote(self):
        ...
        self._draw_rect(
            panel["x"], panel["y"], panel["w"], panel["h"], self.accent_color, 0.1
        )
        ...
        if show_joke:
            ...
            if jokes:
                ...
                for line in lines[:3]:
                    self._draw_text(
                        line, panel["x"] + 15, y_offset, "tiny", self.text_color
                    )
                    y_offset += 20
            else:
    ...
    def update(self, delta_time=None):
        ...
        if self.news_scroll_timer > 30:
            self.news_offset = (self.news_offset + 1) % max(
                1, len(self.data_fetcher.get_news())
            )
            self.news_scroll_timer = 0
        ...
        if self.quote_timer > rotate_interval:
            if self.config.QUOTE_MODE == "alternate":
                max_items = (
                    max(len(self.quotes_list), len(self.data_fetcher.get_jokes()) or 1)
                    * 2
                )
