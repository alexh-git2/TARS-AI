...
class TerminalSystem:
    def __init__(
        ...
        on_display_dashboard: Optional[Callable] = None,
        ...
        self.on_display_dashboard = on_display_dashboard
        ...
        self.max_visible_lines = (
            self.terminal_height - 2 * self.padding - 40
        ) // self.line_height
                ...
                with open(self.log_file, "r") as f:
                    ...
                    for msg in data:
                        self.messages.append(
                            (msg["key"], msg["value"], msg["type"], msg["timestamp"])
                        )
                    self.cache_dirty = True
    ...
    def _save_messages(self):
        try:
            ...
            data = [
                {"key": key, "value": value, "type": msg_type, "timestamp": timestamp}
    ...
    def _init_buttons(self):
        ...
        for button in self.top_buttons:
            ...
            button["rect"] = pygame.Rect(
                x, start_y_top, button_width, button_height_top
            )
        ...
        for button in self.bottom_buttons:
            ...
            button["rect"] = pygame.Rect(
                x, start_y_bottom, button_width, button_height_bottom
            )
    ...
    def scroll_up(self, lines=3):
        ...
        total_lines = sum(
            len(wrapped_lines) for _, _, _, wrapped_lines in self.wrapped_cache
        )
    ...
    def handle_mouse_motion(self, pos: Tuple[int, int]):
        if self.dragging:
            ...
            if lines_to_scroll != 0:
                ...
                scroll_padding = int(self.max_visible_lines * 0.75)
                max_scroll = (
                    max(0, total_lines - self.max_visible_lines) + scroll_padding
                )

                self.scroll_offset = max(
                    -scroll_padding, min(self.scroll_offset, max_scroll)
                )
    ...
    def handle_click(self, pos: Tuple[int, int]):
        ...
        if self.show_power_menu:
            ...
            modal_rect = pygame.Rect(
                self.width // 2 - 200, self.height // 2 - 100, 400, 200
            )
        ...
        for button in self.top_buttons:
            if button["rect"] and button["rect"].collidepoint(pos):
                ...
                elif button["label"] == "DASH":
                    if self.on_display_dashboard:
                        self.on_display_dashboard()
    ...
    def _draw_tech_button(
        self, surface, rect, label, code, active=False, color_type=None, disabled=False
    ):
        if disabled:
        ...
        pygame.draw.line(
            ...
            bracket_color,
            rect.topleft,
            (rect.left + bracket_size, rect.top),
            2,
        )
        pygame.draw.line(
            surface,
            bracket_color,
            rect.topleft,
            (rect.left, rect.top + bracket_size),
            2,
        )
        ...
        pygame.draw.line(
            ...
            bracket_color,
            rect.topright,
            (rect.right - bracket_size, rect.top),
            2,
        )
        pygame.draw.line(
            surface,
            ...
            (rect.right - 1, rect.top + bracket_size),
            2,
        )
        ...
        if disabled:
            ...
            text_color = (
                self.primary_color
                if active or color_type == "warning"
    ...
    def _draw_battery_indicator(self, surface, x, y, width, height):
        ...
        try:
            ...
            pygame.draw.rect(
                surface, self.border_color, (body_x, body_y, body_width, body_height), 2
            )
            ...
            pygame.draw.rect(
                surface, self.border_color, (tip_x, tip_y, tip_width, tip_height)
            )
            ...
            if is_charging:
                ...
                for ox in range(-1, 2):
                    for oy in range(-1, 2):
                        if ox != 0 or oy != 0:
                            offset_points = [
                                (px + ox, py + oy) for px, py in bolt_points
                            ]
                    ...
                    for offset_y in [-1, 0, 1]:
                        if offset_x != 0 or offset_y != 0:
                            ...
                            outline_rect = outline_surface.get_rect(
                                center=(
                                    body_x + body_width // 2 + offset_x,
                                    body_y + body_height // 2 + offset_y,
                                )
                            )
                            surface.blit(outline_surface, outline_rect)

                text_rect = text_surface.get_rect(
                    center=(body_x + body_width // 2, body_y + body_height // 2)
                )
                surface.blit(text_surface, text_rect)
    ...
    def _draw_cpu_temp_indicator(self, surface, x, y, width, height):
        ...
        try:
            ...
            if (
                self.cpu_temp_history
                and len(self.cpu_temp_history) > 1
                and graph_area_height > 0
            ):
                min_temp = 30
                max_temp = 85
                ...
                for i, temp_val in enumerate(self.cpu_temp_history):
                    ...
                    y_pos = (
                        graph_area_y
                        + graph_area_height
                        - (temp_normalized * graph_area_height)
                    )
    ...
    def _draw_power_menu(self, surface):
        ...
        title_rect = title_surface.get_rect(
            center=(modal_x + modal_width // 2, modal_y + 35)
        )
        ...
        for button in self.power_menu_buttons:
            ...
            pygame.draw.line(
                ...
                border_color,
                (rect.right - 1, rect.top),
                (rect.right - corner_size - 1, rect.top),
                3,
            )
            pygame.draw.line(
                surface,
    ...
    def draw(self, surface):
        ...
        pygame.draw.line(
            ...
            (*self.border_color, 200),
            (0, self.toolbar_height - 2),
            (self.width, self.toolbar_height - 2),
            2,
        )
        pygame.draw.line(
            self.overlay_surface,
            (*self.accent_color, 100),
        ...
        terminal_rect = pygame.Rect(
            0, self.toolbar_height, self.width, self.terminal_height
        )
        terminal_bg = pygame.Surface(
            (self.width, self.terminal_height), pygame.SRCALPHA
        )
        terminal_bg.fill((*self.bg_terminal, self.bg_alpha))
        self.overlay_surface.blit(terminal_bg, (0, self.toolbar_height))

        pygame.draw.rect(
            self.overlay_surface, (*self.border_color, 200), terminal_rect, 2
        )
        ...
        pygame.draw.line(
            self.overlay_surface,
            (*self.border_color, 180),
            (10, line_y),
            (self.width - 10, line_y),
            1,
        )
        pygame.draw.line(
            self.overlay_surface,
            (*self.accent_color, 80),
            (10, line_y + 1),
            (self.width - 10, line_y + 1),
            1,
        )
        ...
        if not self.camera_active:
            ...
            terminal_draw_height = (
                self.toolbar_height + self.terminal_height - start_y - self.padding
            )
            ...
            for key, value, msg_type, line_text, line_idx in visible_lines:
                if (
                    ...
                    > self.toolbar_height + self.terminal_height - self.padding
                ):
                    break
                ...
                if line_idx == 0 and ":" in line_text:
                    ...
                    if len(parts) == 2:
                        ...
                        temp_surface = pygame.Surface(
                            (self.width, self.line_height), pygame.SRCALPHA
                        )
                        code_surface = self.font_bold.render(
                            code_text, True, code_color_base
                        )
                        ...
                        temp_surface2 = pygame.Surface(
                            (self.width, self.line_height), pygame.SRCALPHA
                        )
                        temp_surface2.blit(msg_surface, (0, 0))
                        self.overlay_surface.blit(
                            temp_surface2,
                            (x_pos + code_surface.get_width() + 5, y_offset),
                        )
                else:
                    ...
                    self.overlay_surface.blit(
                        temp_surface, (self.padding + 25, y_offset)
                    )
        ...
        pygame.draw.line(
            ...
            bracket_color,
            (term_left, term_top),
            (term_left + bracket_size, term_top),
            bracket_thickness,
        )
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_left, term_top),
            (term_left, term_top + bracket_size),
            bracket_thickness,
        )
        ...
        pygame.draw.line(
            ...
            bracket_color,
            (term_right, term_top),
            (term_right - bracket_size, term_top),
            bracket_thickness,
        )
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_right, term_top),
            (term_right, term_top + bracket_size),
            bracket_thickness,
        )
        ...
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_left, term_bottom),
            (term_left + bracket_size, term_bottom),
            bracket_thickness,
        )
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_left, term_bottom),
            (term_left, term_bottom - bracket_size),
            bracket_thickness,
        )
        ...
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_right, term_bottom),
            (term_right - bracket_size, term_bottom),
            bracket_thickness,
        )
        pygame.draw.line(
            self.overlay_surface,
            bracket_color,
            (term_right, term_bottom),
            (term_right, term_bottom - bracket_size),
            bracket_thickness,
        )
        ...
        for i in range(gradient_height):
            ...
            pygame.draw.line(
                self.overlay_surface,
                (0, 0, 0, alpha),
                (5, gradient_start_y + i),
                (self.width - 5, gradient_start_y + i),
                1,
            )

        bottom_toolbar_y = self.toolbar_height + self.terminal_height
        bottom_toolbar_rect = pygame.Rect(
            0, bottom_toolbar_y, self.width, self.bottom_toolbar_height
        )
        bottom_toolbar_bg = pygame.Surface(
            (self.width, self.bottom_toolbar_height), pygame.SRCALPHA
        )
        ...
        pygame.draw.line(
            ...
            (*self.border_color, 200),
            (0, bottom_toolbar_y),
            (self.width, bottom_toolbar_y),
            2,
        )
        pygame.draw.line(
            self.overlay_surface,
            (*self.accent_color, 100),
            (0, bottom_toolbar_y + 1),
            (self.width, bottom_toolbar_y + 1),
            1,
        )
        ...
        if self.cpu_temp_module and self.show_cpu_temp:
            ...
            self._draw_cpu_temp_indicator(
                self.overlay_surface, cpu_x, cpu_y, cpu_width, cpu_height
            )
