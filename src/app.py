...
if CONFIG["VISION"]["enabled"] == "True":
    ...
    if (
        caps is None
        or caps.can_use_vision
        or CONFIG["VISION"]["server_hosted"] == "True"
    ):
        try:
...
module_servoctl.set_movement_callbacks(
    on_start=pause_ui_and_stt, on_end=resume_ui_and_stt
)
...
def start_discord_in_thread():
    ...
    discord_thread = threading.Thread(
        target=start_discord_bot, args=(process_discord_message_callback,), daemon=True
    )
...
if __name__ == "__main__":
    ...
    stt_manager = STTManager(
        config=CONFIG, shutdown_event=shutdown_event, ui_manager=ui_manager
    )
    ...
    # === Initialize Managers ===
    initialize_managers(
        memory_manager, char_manager, stt_manager, ui_manager, shutdown_event, battery
    )
    ...
    if CONFIG["CONTROLS"]["enabled"] == "True" and BT_AVAILABLE:
        bt_controller_thread = threading.Thread(
            target=start_bt_controller_thread, name="BTControllerThread", daemon=True
        )
    ...
    if CONFIG["CHATUI"]["enabled"] == "True" and CHATUI_AVAILABLE:
        ...
        flask_thread = threading.Thread(
            target=modules.module_chatui.start_flask_app, daemon=True
        )
        flask_thread.start()
    ...
    try:
        ...
        queue_message(
            f"LOAD: TARS-AI v{VERSION} running on {RASPBERRY_VERSION.upper()}{lite_indicator}"
        )
